  ...
  try {
    ...
    const allPracticeIds = [
      ...new Set([
        ...
        ...results.filter((r) => r.orderPractice.practiceId !== null).map((r) => r.orderPractice.practiceId as number),
      ]),
    ];
...
export default async function OrdenDetailPage({
  ...
  const orderPracticeIds = new Set(lines.filter((l) => l.practiceId !== null).map((l) => l.practiceId as number));
          ...
          {lines.length === 0 ? (
            ...
            <div className="overflow-hidden rounded-lg border border-[var(--color-border)] bg-[var(--color-bg-elevated)] shadow-[var(--shadow-xs)]">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {parentLines.map((l) => {
                    const children = l.practiceId ? (childLinesByParentId.get(l.practiceId) ?? []) : [];
                    return (
                        ...
                        {children.map((child) => (
                          ...
                          >
                            ...
                            <td className="py-2 pr-5 text-right">
                              <MoneyDisplay value={child.priceParticular} className="text-xs text-[var(--color-fg-muted)]" />
                            </td>
                            <td className="py-2 pr-5 text-right">
                              <MoneyDisplay value={child.priceInsurer} className="text-xs text-[var(--color-fg-muted)]" />
                            </td>
                            <td className="py-2 pr-5 text-right">
                              <MoneyDisplay value={child.patientCopay} className="text-xs text-[var(--color-fg-muted)]" />
                            </td>
